...
interface BankRecordsSectionProps {
  onNavigateTab?: (tab: string) => void;
  standalone?: boolean;
}

export const BankRecordsSection: React.FC<BankRecordsSectionProps> = ({
  onNavigateTab,
  standalone = true,
}) => {
  const {
  ...
  const [recordToDelete, setRecordToDelete] = useState<BankRecord | null>(null);
  ...
  const [formData, setFormData] = useState({
    ...
    amount: '',
    referenceNo: '',
    category: 'BRANCH_DEPOSIT' as NonNullable<BankRecord['category']>,
  });
  ...
  }, [bankRecords, searchQuery, categoryFilter, dateRangeFilter]);

  // Aggregate stats
  const totalDebits = useMemo(() => {
    return bankRecords.reduce((sum, r) => sum + (Number(r.debit) || 0), 0);
  }, [bankRecords]);

  const totalCredits = useMemo(() => {
    return bankRecords.reduce((sum, r) => sum + (Number(r.credit) || 0), 0);
  }, [bankRecords]);

  const closingBalance = useMemo(() => {
    return bankRecords.length > 0 ? bankRecords[bankRecords.length - 1].balance : ownerTreasury.cashInBank;
  }, [bankRecords, ownerTreasury.cashInBank]);
  ...
  const handleSubmitForm = (e: React.FormEvent) => {
    ...
    if (editingRecord) {
      updateBankRecord(editingRecord.id, {
        date: formData.date,
  ...
  const handleOpenEdit = (rec: BankRecord) => {
  ...
  const handleRequestDelete = (rec: BankRecord) => {
  ...
  const handleConfirmDelete = () => {
    ...
    deleteBankRecord(recordToDelete.id);
    ...
    setActionSuccessMessage(`Bank entry "${details}" was permanently deleted and running balances were updated.`);
  ...
  const handleBulkDeleteConfirm = () => {
    ...
    setActionSuccessMessage(`${count} bank entries were permanently deleted and running balances were recalculated.`);
    setTimeout(() => setActionSuccessMessage(null), 4000);
  };
  ...
  const handleExportCSV = () => {
    ...
    link.setAttribute('download', `bank_records_ledger_${new Date().toISOString().split('T')[0]}.csv`);
  ...
  return (
    <div className="space-y-6" id="bank-records-section">
      {/* Header Banner */}
      <div className="bg-gradient-to-r from-blue-900 via-indigo-900 to-slate-900 rounded-2xl p-5 sm:p-7 text-white shadow-md relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            ...
            <p className="text-blue-200/90 text-xs sm:text-sm mt-1 max-w-2xl">
              Complete chronological ledger of all bank accounts (Zanaco, Stanbic, etc.). Displays every Date, Details, Debit (Deposits), Credit (Withdrawals), and Running Balance.
            </p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            <button
              ...
              className="px-4 py-2.5 bg-blue-500 hover:bg-blue-400 text-white font-bold rounded-xl text-xs sm:text-sm shadow-sm flex items-center space-x-2 transition cursor-pointer"
            >
            ...
            {onNavigateTab && (
              <button
                ...
                className="px-3.5 py-2.5 bg-emerald-500/20 hover:bg-emerald-500/30 text-emerald-300 font-semibold rounded-xl text-xs sm:text-sm border border-emerald-400/30 flex items-center space-x-1.5 transition cursor-pointer"
        ...
        {/* 4 Summary Cards */}
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3.5 mt-6 pt-5 border-t border-blue-800/60">
          <div className="bg-white/10 backdrop-blur-xs rounded-xl p-3.5 border border-white/10">
            <div className="flex items-center justify-between text-xs text-blue-200 font-medium">
              <span>Total Debits (Deposits)</span>
            ...
            <div className="text-[10px] text-blue-300 mt-1">Inflow into Bank accounts</div>
          </div>
          ...
          <div className="bg-white/10 backdrop-blur-xs rounded-xl p-3.5 border border-white/10">
            ...
            <div className="text-[10px] text-blue-300 mt-1">Outflow &amp; Disbursals</div>
          </div>
          ...
          <div className="bg-white/10 backdrop-blur-xs rounded-xl p-3.5 border border-white/10">
            ...
            <div className="text-[10px] text-blue-300 mt-1">Debits minus Credits</div>
          </div>

          <div className="bg-emerald-950/60 rounded-xl p-3.5 border border-emerald-400/40">
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-4 shadow-2xs flex flex-col md:flex-row items-center justify-between gap-3">
        <div className="relative w-full md:w-80">
          ...
          <input
            type="text"
            placeholder="Search details, ref#, date..."
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            className="w-full pl-9 pr-4 py-2 border border-slate-200 rounded-lg text-xs sm:text-sm focus:outline-hidden focus:ring-2 focus:ring-blue-500 bg-slate-50/50"
          />
          {searchQuery && (
            <button
        ...
        <div className="flex flex-wrap items-center gap-2.5 w-full md:w-auto justify-end">
          ...
          <div className="flex items-center space-x-1.5 text-xs text-slate-600 bg-slate-50 border border-slate-200 rounded-lg px-2.5 py-1.5">
            ...
            >
              ...
              <option value="BRANCH_DEPOSIT">Branch Deposits</option>
              ...
              <option value="BANK_FEES">Bank Charges &amp; Fees</option>
          ...
          <span className="text-xs text-slate-500 font-medium px-2">
            Showing <strong className="text-slate-800">{filteredRecords.length}</strong> of {bankRecords.length} entries
          </span>
        </div>
      </div>

      {/* THE 5-COLUMN BANK LEDGER TABLE (DATE, DETAILS, DEBIT, CREDIT, BALANCE) */}
      <div className="bg-white rounded-xl border border-slate-200 shadow-2xs overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse" id="bank-records-table">
            <thead>
              <tr className="bg-slate-900 text-white text-xs font-bold uppercase tracking-wider">
                <th className="py-3.5 px-3 w-10 text-center">
                  <input
                    ...
                    className="w-4 h-4 rounded text-blue-600 border-slate-600 bg-slate-800 focus:ring-blue-500 focus:ring-offset-0 cursor-pointer"
                ...
                <th className="py-3.5 px-4 text-right w-44 text-blue-300">5. BALANCE (K)</th>
              ...
              {filteredRecords.length === 0 ? (
                <tr>
                  <td colSpan={7} className="py-12 text-center text-slate-400">
                    ...
                    <p className="font-semibold text-slate-600">No bank records found</p>
                    <p className="text-xs text-slate-400 mt-1">
                      {searchQuery || categoryFilter !== 'ALL'
                        ? 'Try clearing your search or category filters.'
                        : 'Click "Record Bank Entry" above to add your first transaction.'}
                    </p>
                  </td>
                </tr>
              ) : (
            ...
            <tfoot className="bg-slate-100 border-t-2 border-slate-300 text-xs font-bold text-slate-800">
              <tr>
                <td colSpan={3} className="py-3.5 px-4 font-black uppercase text-slate-900">
                  Total Bank Activity &amp; Closing Balance:
                </td>
                ...
                <td className="py-3.5 px-4 text-right font-mono font-black text-blue-950 text-sm bg-blue-100/70">
      ...
      <div className="p-4 rounded-xl bg-blue-50 border border-blue-200 text-blue-900 text-xs flex items-start space-x-3">
        ...
        <div>
          ...
          <p className="text-blue-800 mt-0.5 leading-relaxed">
            The closing bank balance of <strong>K{closingBalance.toLocaleString(undefined, { minimumFractionDigits: 2 })}</strong> is mathematically bound to the <em>Cash at Bank</em> debit item on the <strong>Business Net Value</strong> statement. Whenever you record deposits or withdrawals here, the Business Net Value updates automatically.
          </p>
        </div>
      </div>
          ...
          <div className="bg-white rounded-2xl max-w-lg w-full shadow-2xl border border-slate-200 overflow-hidden">
            <div className="px-6 py-4 bg-slate-900 text-white flex items-center justify-between">
              <div className="flex items-center space-x-2.5">
                ...
                <h3 className="font-bold text-base">
                  {editingRecord ? 'Edit Bank Transaction' : 'Record New Bank Transaction'}
                </h3>
              </div>
              <button
                onClick={() => {
            ...
            <form onSubmit={handleSubmitForm} className="p-6 space-y-4">
              ...
              <div>
                ...
                <input
                  type="text"
                  placeholder="e.g. Branch Cash Deposit - Lusaka Central Site"
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <input
                    type="text"
                    placeholder="e.g. DEP-ZNCO-88391"
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-3 py-1.5 border border-slate-300 rounded-lg text-xs focus:ring-2 focus:ring-blue-500 focus:outline-hidden font-medium"
                  >
              ...
              <div className="pt-4 flex items-center justify-between border-t border-slate-100">
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <button
                    type="submit"
                    className="px-5 py-2 bg-blue-600 hover:bg-blue-500 text-white font-bold text-xs rounded-xl shadow-xs transition cursor-pointer"
                  >
          ...
          <div className="bg-white rounded-2xl max-w-md w-full shadow-2xl border border-rose-100 overflow-hidden">
            <div className="px-6 py-4 bg-rose-600 text-white flex items-center justify-between">
              <div className="flex items-center space-x-2">
                ...
                <h3 className="font-bold text-base">Delete Bank Record</h3>
              </div>
              <button
            ...
            <div className="p-6 space-y-4">
              <p className="text-slate-700 text-sm">
                Are you sure you want to permanently delete this bank entry?
              </p>
              ...
              <div className="p-3 bg-amber-50 border border-amber-200 rounded-xl text-amber-900 text-xs flex items-start space-x-2">
                ...
                <p>
                  <strong>Recalculation:</strong> Running balances across all subsequent bank transactions will be immediately recalculated and synchronized to the <strong>Business Net Value</strong> statement.
                </p>
              </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/70 backdrop-blur-xs animate-fadeIn">
          <div className="bg-white rounded-2xl max-w-md w-full shadow-2xl border border-rose-100 overflow-hidden">
            <div className="px-6 py-4 bg-rose-600 text-white flex items-center justify-between">
              <div className="flex items-center space-x-2">
                <Trash2 className="w-5 h-5" />
                <h3 className="font-bold text-base">Bulk Delete Bank Records</h3>
              </div>
              <button
            ...
            <div className="p-6 space-y-4">
              <p className="text-slate-700 text-sm">
                Are you sure you want to permanently delete <strong>{selectedIds.length}</strong> selected bank transactions?
              </p>

              <div className="p-3 bg-amber-50 border border-amber-200 rounded-xl text-amber-900 text-xs flex items-start space-x-2">
                <AlertCircle className="w-4 h-4 text-amber-600 shrink-0 mt-0.5" />
                <p>
                  This will remove all {selectedIds.length} records and automatically recalculate closing bank balances and the Business Net Value balance sheet.
                </p>
              </div>
